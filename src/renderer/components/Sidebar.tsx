    ...
    <div className="w-64 min-w-[220px] max-w-[340px] bg-reef-sidebar border-r border-reef-border flex flex-col overflow-hidden">
      ...
      <div className="p-2">
        <div className="relative">
          <svg
            className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-reef-text-dim"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((session, i) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full shrink-0 ${statusDot(session.status)}`}
                        />
                        <span className="text-xs shrink-0" title={session.provider || 'unknown'}>
                          {session.provider
                            ? PROVIDER_BADGE[session.provider]?.emoji || getEmoji(i)
                            : getEmoji(i)}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
                            ...
                            <span
                              className={`text-[8px] font-medium uppercase tracking-wide ${statusColor(session.status)}`}
                            >
